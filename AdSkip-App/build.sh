...
run_candidate() {
  local exe="$1"
  "$exe" "$APP/build.py" > /tmp/adskip_build.log 2>&1
  local rc=$?
  if [ "$rc" -eq 0 ]; then
    return 0
  fi
...
echo "build.sh: all python candidates failed (see /tmp/adskip_build.log)" >&2
exit 1
